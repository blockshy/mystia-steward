...
- `直送酒水`：将当前第一笔普客订单需要的酒水直接写入订单、扣减库存，并尝试触发原生送达表现与状态统计。
- `自动开始料理`：按普客订单所需料理选择可用厨具开始制作。
- `自动收取料理`：料理完成后尝试送达普客订单，不占用玩家送餐盘，并触发游戏订单状态、桌面视觉和经营统计更新。
...
自动化只处理伴随窗口当前排序第一笔稀客订单；普客会按首次出现顺序并发处理最多 3 笔未满足订单，每笔订单独立记录取酒、开始料理和暂停状态。已经进入制作中的普客料理会绑定到目标桌位，后续轮询不会因为同类厨具存在多个而重复开锅。普客酒水和料理会通过游戏订单 setter 写入，并补充通知 `PartnerManager`、`StatusTracker` 与桌面送达视觉，尽量贴近原生送餐流程以触发进餐状态和相关 Buff。厨具占用、材料不足、送餐盘内容不匹配等临时状态会继续等待并重试；建议先在可回退的存档中测试。
