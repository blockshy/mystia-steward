...
- `经营中`：按订单出现顺序查看当前稀客、稀客当前携带金钱、桌位、点单词条、满足点单的推荐料理、推荐加料和推荐酒水；严格满足点单的推荐不足时，会额外显示明确标注的“喜好备选（不满足点单）”。
...
- `设置`：调整窗口背景透明度（20% - 100%）、焦点切换行为、切换冷却时间、始终置顶、主题、手柄导航、缺失厨具过滤、料理/酒水推荐排序、实验性游戏界面置顶、目标厨具高亮，并控制实验性自动化总开关。
...
料理排序默认按 `满足点单 Tag -> 料理分数 -> 加料数量 -> 资源压力 -> 料理售价 -> 加料成本 -> 料理 ID` 执行；酒水排序默认按 `满足点单 Tag -> 酒水分数 -> 酒水售价 -> 酒水 ID` 执行。可在 `设置` 页启用、关闭、调整升降序或移动排序项。排序会影响 `稀客`、`经营中`、稀客订单专注模式，以及自动化处理的当前第一笔订单。
