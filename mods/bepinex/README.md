...
- `自动收取料理`：料理完成后收至对应普客订单的保温箱，不占用玩家送餐盘。
...
自动化只处理伴随窗口当前排序第一笔稀客订单；普客会按首次出现顺序并发处理最多 3 笔未满足订单，每笔订单独立记录开始料理、收至保温箱和暂停状态。已经进入制作中的普客料理会绑定到目标桌位，后续轮询不会因为同类厨具存在多个而重复开锅。普客自动化不处理酒水，也不会把料理直接送达订单；料理收至保温箱后，需要玩家按游戏原生流程送达。厨具占用、材料不足、运行时对象暂不可读等临时状态会继续等待并重试；建议先在可回退的存档中测试。
