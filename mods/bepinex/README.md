...
- `经营中`：查看当前稀客、稀客当前携带金钱、桌位、点单词条、满足点单的推荐料理、推荐加料和推荐酒水；订单默认按点单先后排列，也可在 `设置` 页切换为稀客分组，严格满足点单的推荐不足时，会额外显示明确标注的“喜好备选（不满足点单）”。页面底部会显示普通客订单诊断；开启实验性自动化总开关和“普通客订单处理”子开关后，可手动处理第一笔普通客订单。
