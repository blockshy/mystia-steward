...
- `经营中`：查看当前稀客、稀客当前携带金钱、桌位、点单词条、满足点单的推荐料理、推荐加料和推荐酒水；订单默认按点单先后排列，也可在 `设置` 页切换为稀客分组，严格满足点单的推荐不足时，会额外显示明确标注的“喜好备选（不满足点单）”。页面底部会显示普通客订单诊断，用于后续自动化验证。
- 料理和酒水推荐行可点击 `收藏`。同一稀客再次出现相同料理或酒水点单词条时，可用收藏会优先显示；再次点击可取消收藏。
- `稀客订单专注模式`：只显示当前点单推荐；可切换精简模式，并自行设置推荐料理/推荐酒水显示数量，没有点单时显示等待提示。
- `任务`：读取当前进度中可接但未接的交互任务，显示任务标题、角色和读取来源。
