...
- `GET /orders/normal/complete-first?...`：处理当前第一笔普客订单。普客自动化只负责按订单料理开始制作，并在完成后把料理写入该订单的 `ServedFoodInAir` 保温位；不处理酒水，不写入 `ServFood/ServBeverage`，也不触发订单评价。
...
普客订单自动化仍是实验性功能。伴随窗口会显示当前 UI 订单里识别到的普客桌位、料理、酒水和完成状态；设置页开启自动化总开关后，还需要在经营中自动化面板开启“启用普客处理”，并至少开启自动开始料理或自动收取料理中的一个阶段，之后会自动处理按首次出现时间排序的未满足普客订单，不再需要点击手动处理按钮。普客流程只制作料理并收至普客保温箱，最终送达和酒水处理保留给玩家走游戏原生操作，避免绕过游戏进餐动画、Buff 和订单状态机。
