...
- `GET /orders/normal/complete-first?...`：处理当前第一笔普客订单。酒水会通过订单送达状态写入并扣减库存；料理会按首次出现的普客订单绑定目标，完成后写入订单，不占用玩家送餐盘；送达后会补充 `PartnerManager`、`StatusTracker`、桌面视觉和未完成订单耐心恢复等原生副作用，订单满足后才触发游戏评价流程。
...
普客订单自动化仍是实验性功能。伴随窗口会显示当前 UI 订单里识别到的普客桌位、料理、酒水和完成状态；设置页开启自动化总开关后，还需要在经营中自动化面板开启“启用普客处理”，并至少开启直送酒水、自动开始料理或自动收取料理中的一个阶段，之后会自动处理按首次出现时间排序的未满足普客订单，不再需要点击手动处理按钮。普客酒水和料理送达时必须优先调用订单属性 setter，并补齐 `PartnerManager.OnOrderBaseStatusUpdate` / `NotifySystemChanged` 的 `BeverageDelivered` 或 `FoodDelivered` 上下文、`StatusTracker.AddBussinessFoodConsumes/AddBussinessBeverageConsumes`、`GuestTableDisplayer.SetFoodVisual/SetBeverageVisual` 与未完成订单耐心恢复，避免绕过游戏的桌面表现、进餐状态和 Buff 逻辑。
