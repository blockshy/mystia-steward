...
伴随窗口的自动化能力只在前端 `设置` 页总开关开启后运行，且只处理当前排序第一笔稀客订单。料理/酒水排序配置会影响稀客页、经营中页、专注模式和自动化选单，新增排序项时需要同时覆盖这些入口。子选项默认关闭并单独记忆：自动完成订单、自动取酒、自动开始料理、自动收取料理、只处理收藏配方和出错暂停。临时失败例如厨具占用、送餐盘缺少目标成品、运行时对象暂不可读，应保持可重试，不应永久停止自动任务；非临时错误在 `出错时暂停` 开启时才暂停当前订单。
