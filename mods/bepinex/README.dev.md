...
普客订单自动化仍是实验性功能。伴随窗口会显示当前 UI 订单里识别到的普客桌位、料理、酒水和完成状态；设置页开启自动化总开关后，还需要在经营中自动化面板开启“启用普客处理”，并至少开启送达酒水、自动开始料理、自动收取料理、自动送达料理或自动完成订单中的一个阶段，之后会自动处理按首次出现时间排序的未满足普客订单，不再需要点击手动处理按钮。普客流程允许把酒水直接写入订单、把完成料理先收至游戏料理暂存容器，再从暂存容器写入订单并调用 `EvaluateOrder()`；该链路用于适配需要真正订单评价的经营挑战。`ServedFoodInAir` / `ServedBeverageInAir` 用于触发订单 UI 更新，正式完成仍以 `ServFood` / `ServBeverage` 和 `get_IsFullfilled()` 为准；`CookController` 不是独立保温箱。
