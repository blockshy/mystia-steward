...
- `任务` 页读取当前进度可接但未接的交互任务，优先使用 `DataBaseDay.GetAllNPCKeys()` 和 `RunTimeScheduler.GetAvailableInteractMissionForCharacter()`；读取失败必须显示诊断信息，不得回退到静态全任务列表误导用户。
- 普通客订单自动化必须先经过只读诊断阶段。当前可在经营中页展示 `OrderController.GetShowInUIOrders()` 读取到的普通客订单、桌位、料理、酒水和完成状态；不要在未确认保温箱、直接送达和订单状态更新入口前接入自动执行。
