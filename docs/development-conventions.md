...
- 普客订单自动化必须建立在只读诊断可识别订单的前提下。经营中页诊断和执行路径都需要复用健壮的 IL2CPP 枚举与单例查找逻辑，至少读取 `OrderController.GetShowInUIOrders()`、HUD `OrderingElement.ActiveOrder`、`GuestsManager` 的 Presented/Desk/DeskMap/Repellable/ManualDesk 控制器以及 Queue 控制器中的 `AllOrders`/`AllOrdersData`/`PeekOrders()`。当前普客执行入口按首次出现顺序并发处理最多 3 笔未满足订单：酒水可写入订单并扣库存，料理完成后绑定到当时的目标订单，不得收入玩家送餐盘；酒水和料理送达不能只写 `OrderBase.ServFood/ServBeverage`，还必须补齐 `PartnerManager` 的 `BeverageDelivered` / `FoodDelivered` 状态更新、`StatusTracker.AddBussiness*Consumes`、桌面 `GuestTableDisplayer` 视觉更新，并在订单未满足时恢复对应客人的耐心，避免绕过游戏进餐状态、桌面表现和 Buff 逻辑；订单满足后才调用 `EvaluateOrder`。每笔普客订单需要独立记录已取酒、已开始料理和暂停状态；同一订单已有 pending 料理时只能等待，不能因场景里有多个同类厨具而重复制作。
