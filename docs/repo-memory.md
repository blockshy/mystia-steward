...
- 本地 API 快照发布在 Unity 主线程节流到约 0.5 秒一次；完整 `RuntimeDataCatalog` 只在首次、变化、强制刷新或约每 10 秒补发，其余快照可省略 `runtimeData`。前端必须缓存最近一次完整运行时数据。`performanceMs` 会记录 `refresh.business`、`refresh.runtime`、`snapshot.serialize`、`automation.collect`、`snapshot.publish` 等耗时，概览页和经营中页展示最高几项用于排查掉帧。
